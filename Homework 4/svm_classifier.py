...
def load_data(filepath):
    ...
    features = []
    labels = []
    has_labels = 'train' in filepath.lower()
...
test_features, test_labels_none = load_data('test.txt')
...
def main():
    ...
    # 由于测试集没有标签，我们创建一个假标签用于演示
    # 在实际作业中，如果测试集有标签，应该使用真实标签
    print("\n注意: 测试集没有提供标签")
    print("为了演示正则化分析，我们将使用训练集的一部分作为验证集")
    ...
    save_results_summary(results, X_train_split, X_val)
